...
R2. `/tests/support` is the only test-framework code (doctest-style micro framework); it MUST stay Qt-free and dependency-free. All tests link into the `piricad_tests` executable.
...
R8. A benchmark more than 10% worse than the stored baseline MUST break the build (§10.1).
