...
> Scope: `/src/plugin-api`, `/tests/unit/plugin_*`, `/scripts/ci-gate-abi.sh`, `/scripts/ci-gate-visibility.sh`  |  Depends on: `kentos_command` (via the bus only), `kentos_core` types by value only — never Qt, never `kentos_app`  |  Source: kentoscad.md §4.3, §2.5, §2.6, §1, §8, §11 (Faz 5), §13

The public surface is ONE header: `/src/plugin-api/include/kentos_cad_plugin.h`, built as CMake target
`kentos_plugin_api`. It does not exist in Phase 0; these rules bind the first commit that creates it.

## Hard Rules

**R1** — The whole boundary SHALL be C99: `extern "C"`, fixed-width integers, `const char*` UTF-8,
opaque handle typedefs (`kentos_host*`, `kentos_plugin*`), and POD structs with explicit padding.

**R2** — Coordinates SHALL cross as `int64_t` millimetres, bit-identical to `Mm` in
`kentos_cad/core/units.hpp`. Doubles MUST NOT be used for stored geometry on the boundary.

**R3** — Every plugin SHALL export `uint32_t kentos_plugin_abi_version(void)` and
`int kentos_plugin_init(const kentos_host_v1*, kentos_plugin**)`. The host SHALL run the
...
the END of `kentos_host_vN`, never inserted; every versioned struct SHALL carry `uint32_t size` as
...
`host->submit(cmd_id, argv, argc, kentos_error*)` where arguments serialise to `Value`
(`kentos_cad/command/value.hpp`). Read APIs may be rich and direct, but read-only (§4.3).
...
**R7** — Every entry point SHALL be `noexcept` and report failure as `kentos_error {int32_t code;
const char* message;}` mapping 1:1 onto `Error{code,message}` from `kentos_cad/core/result.hpp`. The
...
**R13** — `kentos_plugin_api` SHALL build with `-fvisibility=hidden -fvisibility-inlines-hidden`
(MSVC: no auto-export) and export exactly the symbols listed in
`/src/plugin-api/kentos_plugin_api.exports`.
...
longer, and is marked `KENTOS_DEPRECATED` from the release that deprecates it.

## Absolute Prohibitions

**P1** — NEVER put a C++ class, template, reference, STL container/string, `Result<T>`, `Task<T>`, or
any `<Q...>` type in `kentos_plugin.h`. BANNED without exception.
...
**P9** — NEVER link Qt, GDAL, or any GPL-incompatible library into `kentos_plugin_api`, and NEVER
put an `#if`/`#ifdef` on a `KENTOS_*` macro inside `kentos_plugin.h`.

## Definitions of Done

- [ ] `kentos_plugin.h` compiles clean as C99 with `-Wall -Wextra -Werror -pedantic`.
...
| Gate | Mechanism |
|---|---|
| ABI compatibility | `scripts/ci-gate-abi.sh` — `abidiff` current `kentos_plugin_api` vs previous release tag; non-additive diff fails CI |
| Symbol visibility | `scripts/ci-gate-visibility.sh` — `-fvisibility=hidden` asserted in CMake, `nm -D` diffed against `kentos_plugin_api.exports` |
| No C++/Qt in header | `scripts/ci-gate-abi.sh` greps `kentos_plugin.h` for `class`, `template`, `std::`, `#include <Q`, and `#if`/`#ifdef KENTOS_`; any hit fails (P1, P9) |
