...
R20a. **The numerical contract has five quantities and each has ONE home** (`kentos_cad/core/precision.hpp`, TODOS F-03): the storage resolution is the millimetre (`Mm`, not a tolerance); the COMPUTATION thresholds — on a curve within `kOnCurveMm` (0,5), one point within `kSamePointMm` (1), a fitted circle within `kRoundedFitMm` (1,5), a stored arc consistent within `kStoredArcMm` (2) — are named constants derived from it and never settings, and a literal restating one is a defect; the SCREEN tolerances are pixels (`core.yakalama.tolerans`, `core.secim.tolerans`) and act on an aimed point only (command.md R9a); the TOPOLOGY tolerance is the Project setting `core.topoloji.dugum_toleransi`; the EXPORT chord tolerance is the Project setting `core.aktarim.egri_sapmasi`, honoured by `core::stroke_curve` for every curve kind a file cannot hold. A measured length or area of a curve MUST come from the curve (`path_length`, `path_area`, closed forms), never from the picture's fixed-density chords. There is ONE area rule: exact in 128 bits about the first vertex, halved half away from zero (`signed_ring_area`, `RingGeometry::ring_area`, `path_area` agree to the square millimetre), and `RingGeometry::append` refuses a ring whose doubled area does not fit `Mm2` rather than store a wrapped figure. A 128-bit result is brought back to 64 bits by `saturate_int64`, never by a narrowing cast.
