...
> Scope: `/src/core` (CMake target `piricad_core`), plus its tests in `/tests/unit`, `/tests/golden`, `/tests/bench`  |  Depends on: **nothing** — not Qt, not any other PiriCAD module  |  Source: piricad.md §7.1, §7.3, §8, §9.1, §9.2, §10.2, §10.5, §13

## Hard Rules

R1. `piricad_core` MUST stay a static, Qt-free library; its `target_link_libraries` list SHALL contain only third-party libraries from §9.1/§9.2 — no `Qt6::*`, no other `piricad_*` target (§8).
R2. Every stored coordinate MUST be `Mm` (`int64` fixed-point millimetres) from `piricad/core/units.hpp`; `Point2` and `Box2` SHALL hold `Mm` members only (§10.2).
...
R8. Orientation, incircle and segment-side tests MUST route through the Shewchuk `predicates.c` wrapper compiled into `piricad_core` (§9.2).
...
R10. Every fallible function MUST return `Result<T>` from `piricad/core/result.hpp` with a populated `Error{code,message}`; the happy path SHALL NOT be signalled by a sentinel value (§7.1).
...
R17. Every public header under `/src/core/include/piricad/core` MUST compile standalone under `-Wall -Wextra -Werror` on GCC 13+, Clang 17+ and MSVC 19.40+ (§7.2).
...
R19. `Crs` in `piricad/core/crs.hpp` MUST remain a value type (identifier plus parameters, e.g. `TUREF/TM30`); executing a transformation requires PROJ and belongs outside core — see `.claude/io.md` and `.claude/domain.md`.
R20. Rounding of a transient `double` to `Mm` MUST use the one shared half-away-from-zero rounding helper declared in `piricad/core/units.hpp`; `static_cast<int64_t>`/`static_cast<Mm>` on a coordinate `double` is banned everywhere under `/src` outside that helper's own definition (§7.3).
...
- [ ] `nm`/`dumpbin` on `piricad_core` shows no Qt symbol and no symbol from another `piricad_*` target.
...
- **`/tests/unit`** — built into the `piricad_tests` executable; covers `Result<T>` error paths, `operator<=>` totality, arena reuse, index bulk-load.
- **`/tests/bench`** — benchmark gate; >10% regression breaks the build (§10.1).
- **CMake** — `piricad_core` fails to configure if a `Qt6::*` or upward `piricad_*` target appears in its link interface, or if the float flags of §7.3 are absent.
