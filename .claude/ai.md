...
R24. **An external client NEVER dispatches.** A tool call naming a command that lacks `Flags::NoEffect` is compiled into an `ai::Plan`, answered IMMEDIATELY with the plan's id, its state and the exact command lines it holds, and applied only through `core.suggestion uygula` taken by a person at this workstation. The answer MUST say so in Turkish; an agent that reads an empty result as a failure retries, and eleven duplicate suggestions on somebody's screen is the cost. (R3, R4 and CLAUDE.md 5.7 in the remote case, which the maintainer confirmed explicitly.)
...
P15. NEVER let an approval be forged. `ai::Approval` has a private constructor and one factory, `ai::Gate::approve`, called from the suggestion card and nowhere else; a token, a header, a setting or a command-line flag that stood in for that call would be exactly the "trust mode" P1 forbids. `scripts/ci-gate-ai.sh` fails the build on a second caller.
