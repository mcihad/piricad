...
R8a. A DRAWING's text — every caption the document carries — MUST be set in technical spacing (`render::Spacing::Technical`): no kerning pair and no ligature, and every cluster placed where `core::text_run_advance` of the text before it ends, so what the canvas and the QPainter path (the PDF and the printer) draw is exactly `core::text_width` (`.claude/core.md` R21) — the width its box, its dimension fit and its line breaks were decided by. Both backends lay a caption out with `render::drawing_measure`; a wrapping text arrives already broken by `core::text_lines`; the QPainter path draws the atlas's own glyph choice from the font file (`TextAtlas::glyphs`), never Qt's shaping of the string. The program's own labels (a ruler's figures, the canvas notes) may be typeset (TODOS C-18).
...
R15. Render MUST read the `Document` through a read-only snapshot and consume its SoA polyline store directly; state changes only ever come back as commands (`core.zoom` and friends) — see `.claude/command.md`. A kind that is not its vertices is read through `core::entity_outline` as `EmitBuffer` RUNS (`run_closed`, `run_hole`, and `run_style`/`run_layer`/`run_text` for a block member's own look and caption, batched where THEY say), one code path for every such kind; a block reference expands there per frame, a captioned kind (a dimension) draws its caption and its runs, and a tessellation cache keyed `(revision, BlockId, transform)` is later work.
