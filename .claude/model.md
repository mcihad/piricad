...
R36a. **A document's CRS counts metres.** `Crs::unit()` (`CrsUnit::Metre | Degree | Other | Unknown`) is what one coordinate of the system counts; the geodesy module fills it through `Bus::on_crs_resolve` by asking PROJ (the zone catalogue and `YEREL`/`LOCAL` are metres by definition), and `/src/io` fills it from GDAL for a file's layer. Because the store holds millimetres, a system that counts degrees, feet or a geocentric X/Y/Z MUST NOT become a document's CRS: `AYAR koordinat_sistemi` and `OTURT sistem=` refuse one BEFORE anything moves, and a project saved with one opens with a warning rather than a lock-out. `Unknown` is let through — an id nobody can place is the user's to name. `core::crs_unit_problem` is the one message every door uses. The drawing unit `core.cizim.birim` is NOT the CRS unit: it is what a CAD file's numbers mean at the file boundary and nothing else (TODOS F-03).
...
R46e. A **RESULT** is a lineage that can go out of date (TODOS F-04): an analysis output that is a statement ABOUT its sources — a buffer (`islem.tampon`), an area generated from lines (`islem.alan_uret`), a boundary (`core.boundary`), a contour run (`core.contour`) — records, beside each source, the source's CONTENT REVISION when it was computed (`Lineage::revisions`, one per source, in source order; `Document::content_revision`: kind, payload, rings, caption and every PRESENT cell with its column id — never layer, style, flags or key, and a column declared later changes no revision). It is recorded through `Context::derive_result` / `derive_results`, never by a client; a copy, a piece, an ifraz parcel stay history (R46d), and a caption that FOLLOWS its source (R46) records history, because R46a keeps it current. Whether a result is current is COMPUTED, never stored: `core::check_origin` compares each recorded revision with the source now — `Current`, `Stale` (a live source changed), `Sourceless` (only gone ones) — so an undo that restores the source makes the result current with no flag to find. `Transaction::settle_results()` runs last at commit (after R46a's settles), asks ONLY the results made from what the range reshaped, re-worded, re-valued or erased, one comparison per shared origin, and reports; the one thing it writes is for a result that was ITSELF reshaped: carried with every live source it has, it is re-recorded against them (still current); reshaped on its own, it is RELEASED — revisions cleared, history kept — like a dimension dragged off its feature (C-10) and a hatch moved off its boundary (C-11). One run shares ONE origin record: `LineageTable` pools equal origins (append-only, like the geometry arena), and the file writes a result's origin once per run in blocks `0x0097` (`ResultOriginRecord`), `0x0098` (`ResultSourceRecord`: key + revision, per origin a contiguous run) and `0x0099` (`ResultRowRecord`), all three or none, only when a result exists; history stays in `0x0096`, so a drawing without a result keeps its bytes. The undo record of a result's lineage is layout 2 of `encode_lineage` (revisions after sources); layout 1 is byte-for-byte what it was. A result's revisions fold into `content_hash()` only when present. A cycle cannot form: a source must exist before its result (`Document::set_lineage` refuses a key not yet issued and the object itself).
R46f. **A follower behind a lock keeps its tie true** (TODOS F-04). A caption (R46), a linked dimension (C-10) and a linked hatch (C-11) that may not be edited are LEFT where they are when their source changes (R46b) — but their TIE is not: a corner renumbered on the source re-anchors the attachment, renumbers the dimension's link, and a source erased breaks the dimension's or the hatch's link, through `Document::retie_attachment` / `retie_dimension` / `retie_hatch`, which change only the anchor numbering and the broken mark, never which object is followed and never the follower's geometry, words or style, and so do not ask the lock; the settle is their only caller. A follower is BEHIND when it no longer stands where its tie puts it: `core/ties.hpp` answers for every tie kind (`caption_follow` is the ONE home of a caption's rule place — the settle writes exactly what it returns, and the check compares with it), computed from the drawing and never stored, like R46e. `Transaction::settle_unlocked` runs at commit after the three follower settles: a follower this range set free — its layer unlocked (`Op::Kind::SetLayerLocked` whose inverse re-locks), or it moved to another layer — that is behind is brought to its source in the same undo step (`follow_caption` / `follow_dimension` / `follow_hatch`, the ones `BAĞIMLILIK islem=yenile` calls too), and the follower settles' cursors are advanced past those writes so none reads a caught-up follower as one moved by hand. `Document::restore_attachment` is the undo half of `set_attachment` and asks no lock, like `restore_dimension_links` and `restore_hatch_links`. A hatch's sources may not come back to it through other hatches (`Document::set_hatch_links`); a dimension links only to lines, polylines, points, arcs and circles, so it cannot close a cycle.
