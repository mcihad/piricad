...
R12. Every OPTIONAL external dependency MUST sit behind a `KENTOS_WITH_<NAME>` option defaulting to `OFF` (GDAL, PROJ, GEOS, CGAL, LUA, PYTHON, TRACY, IMGUI, …). ON but missing MUST `message(FATAL_ERROR)` naming the package, the vcpkg port and the install command.
