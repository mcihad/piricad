...
> Scope: `/CMakeLists.txt`, `/cmake`, `/CMakePresets.json`, `/Makefile`, `/vcpkg.json`, `/packaging`, `/.github/workflows`, `/scripts/ci-gate-*.sh`  |  Depends on: every `kentos_*` target consumes this, it consumes none  |  Source: kentoscad.md §7.1, §7.2, §7.3, §8, §9.11, §13, §14
...
R4. The target set is exactly `kentos_core`, `kentos_command`, `kentos_io`, `kentos_domain_geodesy`, `kentos_domain_cadastre`, `kentos_domain_planning`, `kentos_domain_surface`, `kentos_script`, `kentos_ai`, `kentos_render`, `kentos_plugin_api`, `kentos_app` (executable `kentos_cad`), `kentos_tests` (CLAUDE.md 3.2–3.4). Phase 0 builds only `kentos_core`, `kentos_command`, `kentos_script`, `kentos_render`, `kentos_app`, `kentos_tests`; the rest land with their modules. A target outside this list requires an edit to this rule first.
R5. Every target not in CLAUDE.md 3.4's Qt-linked list (`kentos_render`, `kentos_app`) MUST NOT list any `Qt6::*` in `target_link_libraries` (see `.claude/core.md`).
...
R8. One compiler and one standard library per platform; all dependencies MUST be built with the same toolchain as KentOSCad (§7.2).
...
R12. Every OPTIONAL external dependency MUST sit behind a `KENTOS_WITH_<NAME>` option defaulting to `OFF` (GDAL, PROJ, GEOS, CGAL, PYTHON, TRACY, IMGUI, …). ON but missing MUST `message(FATAL_ERROR)` naming the package, the vcpkg port and the install command.
R12a. The mandatory dependencies — Qt 6, Qt Advanced Docking System (§9.4), HarfBuzz, FreeType, msdfgen — MUST be `find_package`d unconditionally with a pinned minimum version and MUST hard-fail when absent; they carry no `KENTOS_WITH_*` option. **SARibbon** (ui.md R46) is mandatory for `kentos_cad` in the same sense and is the one fetched rather than found: no port exists, so it is a `kentos_dependency` pinned by commit SHA, populate-only, and built from its one-file amalgamation as a static target inside `/src/app` (never linked into a Qt-free target).
...
R14. All `kentos_*` targets MUST build with `/W4 /WX` (MSVC) or `-Wall -Wextra -Wpedantic -Werror` (GCC/Clang).
...
R18. Every release job MUST emit a CycloneDX SBOM artifact `kentoscad-<version>-sbom.cdx.json` listing each linked dependency with resolved version and SPDX license id (GPL compliance requires it) (§9.11, §13).
...
R21. Versioning MUST be SemVer from the single source `project(KentOSCad VERSION …)`, and `/docs/api-stability.md` MUST be updated in the same PR that changes `/src/plugin-api` (§13).
R22. `/Makefile` MUST be a thin wrapper: one documented line per target calling `cmake --preset`, `cmake --build --preset`, `ctest --preset`, or a `/scripts` script. `make check` MUST run format + tidy + IWYU + every `scripts/ci-gate-*.sh` + `kentos_tests`.
R23. Qt MUST ship bundled in the base package (windeployqt / macdeployqt / AppImage payload) at the version pinned in `/vcpkg.json`. The embedded CPython 3.14 runtime MUST ship only inside the separate, optional Python module package — never in the base installer (§4.2; `.claude/script.md` R7, P13). (3.14 supersedes the 3.12 this rule pinned, with §4.2 and `script.md` R7.)
...
P11. NEVER default a `KENTOS_WITH_*` option to ON, and NEVER silently fall back to a stub when the package is missing.
...
- [ ] `make check` is green (format, tidy, IWYU, all `ci-gate-*`, `kentos_tests`).
- [ ] Zero warnings; no new `-Wno-*` or pragma disable in the diff.
- [ ] New dependency: pinned in `/vcpkg.json`, gated by `KENTOS_WITH_*` default OFF, license recorded in `/NOTICE`, SBOM regenerated.
