...
R9a. The Python surface is PROJECTED, never written: one callable per `Scriptable` command, built from `Registry` at run time, with English keywords taken from `Param::english` and a callable name from `command::python_callable_name`. Hand-written names live where the projection cannot reach — `cad.doc` for the document reads — and `bind_commands` MUST refuse to shadow an existing attribute rather than winning silently (CLAUDE.md 5.20, 6.15).
...
R11a. WHAT THE LEVEL GOVERNS IN AN EMBEDDED CPYTHON, stated because the runtime cannot keep a stronger promise: the scope of the BINDINGS, the interpreter's isolation (`PYTHONPATH`, `PYTHONHOME` and `PYTHONSTARTUP` unread, the script's own directory off `sys.path`, no user site directory), the consent `tam` requires, and the journal record. It is NOT a jail around the interpreter — `os`, `socket`, `subprocess`, `ctypes` and `__import__` are in the one CPython and `import os` works at `güvenli`. A rule claiming otherwise would offer a guarantee the runtime cannot keep, and the documentation says so to the user in the same words (`docs/betik/python.md`).
...
P15. NEVER give `core.script`, or any future command that runs a script, the `AiAccessible` flag (CLAUDE.md 5.24). An agent proposes commands, which are previewable, validated and journalled one at a time; a script is none of those things until it has already run. `tests/unit/test_command.cpp` asserts the bit is absent, so setting it fails the build.
