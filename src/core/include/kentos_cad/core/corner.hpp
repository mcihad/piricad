...
// vertices and not bulges (model.md R9-R12): the arc is a real arc with a real
// centre and radius, so its length and geometry are exact.
#pragma once
...
struct CornerCut
{
    ...
    /// corner replaced by its two tangent points; for a fillet the leg up to the
    /// first tangent point.
    ...
    bool arc{false}; ///< a fillet: the arc below joins the two legs
...
/// when the corner is straight, when the cut would run past a neighbouring
/// vertex, and — for a fillet — when the run is closed: a boundary that is partly
/// an arc is not a ring this model can hold (model.md R9-R12).
