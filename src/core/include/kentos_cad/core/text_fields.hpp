...
//
// IN `core`, because the picture fills fields too: a block's member caption
// that names `{no}` is drawn with the number of the REFERENCE it is drawn by
...
struct FieldFormat
{
    ...
    DrawingUnit unit{DrawingUnit::Metre}; ///< for `{#cevre}` and `{#uzunluk}`
};
...
/// The COLUMNS `format` names — `{no}` gives `no`; a `{#alan}` figure names no
