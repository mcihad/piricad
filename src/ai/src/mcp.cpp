...
{
    // THE SHARED COMPILER, so the chat and this server accept exactly the same
    // arguments and refuse exactly the same ones (`ai/arguments.hpp`).
    return compile_arguments(spec, arguments, dispatcher_.handles(requester),
                             dispatcher_.revision());
}
