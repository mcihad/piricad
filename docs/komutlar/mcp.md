...
Bir yapay zeka ajanının programa bağlanmasını isteyen, sonra bağlantıyı kapatmak ya da
erişim belirtecini yenilemek isteyen harita mühendisi için; bu sayfayı bitirdiğinizde
sunucuyu başlatmayı, durdurmayı, durumunu okumayı ve yeni bir belirteç üretmeyi
bileceksiniz.
...
başlatır, durdurur, durumunu söyler ya da yeni bir **erişim belirteci** üretir.
...
| `belirtec` | **Yeni** bir belirteç üretir; eskisi o anda geçersizleşir |
...
| Parametre | Ne yapar |
|---|---|
| `islem` | Yapılacak iş. Zorunlu. `baslat`, `durdur`, `durum` ya da `belirtec` |
| `port` | **Yalnız bu başlatma için** port: 1024–65535. Verilmezse ayardaki port (öntanımlı **8765**) kullanılır. `durdur`, `durum` ve `belirtec` ile anlamı yoktur |
...
**Başlat** / **Durdur**, **Yeni belirteç üret**, **Adresi kopyala**. Belirtecin tamamı
yalnız buradadır; komut satırı yalnız ilk altı hanesini yazar. Sayfa, adresin bir parola
olduğunu da söyler: belirteci taşır.
...
| `Tanınmayan işlem: 'ac'. İşlemler: baslat / durdur / durum / belirtec` | `islem` dört sözcükten biri değil (komut satırında sorulan işlem) | Dört sözcükten birini yazın |
| `'core.mcp': 'islem' için tanınmayan değer 'ac'. Kabul edilenler: baslat / durdur / durum / belirtec` | Aynı hata, betikten geldiğinde: veri yolu gövde çalışmadan önce yakalar | Dört sözcükten birini yazın |
...
| `'core.mcp': bilinmeyen parametre 'adres'. Tanımlı parametreler: islem, port` | Tanımlı olmayan bir parametre adı | `islem` ve `port` dışında parametre yoktur; adres her zaman `127.0.0.1`'dir |
