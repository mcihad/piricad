...
| Katman süzme | ~~Aktarılmıyordu~~ **Düzeltildi (19 Eylül 2026)**: `render::SceneOptions::layer_allowed` eklendi — katman başına bir bayt, çünkü sahne bunu nesne başına bir kez okuyor ve orada bir ad karşılaştırması kare bütçesinin içine girerdi (§10.1). `paint_map` maskeyi `item.layers`'tan kuruyor. Boş olması "görünür bütün katmanlar" demek ve maske **daraltıyor, genişletmiyor**: belgenin gizlediği katman ne olursa olsun gizli kalıyor (model.md R7). | **Kalan:** `layers` alanı hâlâ hiçbir komuttan AYARLANAMIYOR — dosyadan ve şablondan geliyor. `katmanlar=` parametresi bir metin listesi ister ve `Value`'da öyle bir tür yok (`Kind::{Empty,Bool,Int,Number,Text,Point,PointList,IdList}`). Virgülle ayırmak belirsiz olurdu: katman adı doğrulanmıyor, virgül içerebilir. **`Value::Kind::TextList` L-04'ün parçası olarak eklenmeli** — JSON gidiş-dönüşü, günlük baytları ve ajan şeması dahil. |
