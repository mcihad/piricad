...
- [ ] **C-09 · P1 — Alan üretimi, sınır bulma ve geometri temizliği.** Kapalı bölgeyi tıklayarak sınır çıkarma, delik/ada tanıma, çizgi ağından alan üretme, yinelenen ve sıfır uzunluklu öğeleri bulma.
  ...
  **İlerleme (24 Eylül 2026) — 1. aşama, sınır bulma:** Çekirdek `core/planar.hpp` (CGAL `Arrangement_2`, daire-doğru parçası izleri; kesin aritmetik, yaylar yay kalır). `SINIR` komutu, önizlemesi ve araç düğmesi. Kabulün ilk iki cümlesi sınandı: *açık uçlar sessiz kapanmaz, gösterilir* — `test_planar.cpp` "yakın ama açık uç sessizce kapanmaz", "5 cm açık kalan köşe …", tuvalde `MeasureMark::Gap`, gerçek fare probu 11. bölüm, kare `17b-sinir-bosluk`; *iç ada delik olarak korunur* — "içerideki kapalı çizgi ada olur", "sınıra bir çizgiyle bağlı ada da delik olur", kare `17a-sinir-onizleme`; eşitlik kanıtı `PROOF: SINIR`. **Kalan:** 2. aşama çizgi ağından toplu alan üretimi; 3. aşama yinelenen/sıfır uzunluklu öğe bulma ve onarım raporu, TOPOLOJİ'nin aynı çekirdeği kullanması.
