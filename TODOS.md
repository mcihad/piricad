...
| Kes/uzat/böl | **Ölçüldü:** kes/uzat/böl/kır/uç uca yalnız çizgi ve köşeli çoklu çizgide çalışıyor; yay, daire, elips, spline, yaylı çizgi, tarama ve liderde reddediyor. Delikli alanda da kes/böl/kır reddediyor. **Güncelleme (23 Eylül 2026, C-04 ilk adım):** BUDA yay ve dairede, UZAT yayda çalışıyor (matris: daire BUDA ✓, yay BUDA/UZAT ✓); iki sınır arasındaki orta parça atılıyor, çok sınır ve hızlı budama var. BÖL/KIR/UÇUCA eğride hâlâ reddediyor (C-05); elips/spline C-01'in yinelemeli çözümünü bekliyor | Yay, daire, elips ve spline için türüne uygun işlemler |
...
- [ ] **C-04 · P0 — Eğrilerde ve toplu seçimde kes/uzat.** Bir veya çok sınır seçimi, tutulacak bölüm, ters taraf ve geçici sınır uzantısı desteklensin; sınır ve hedef nesne rolleri net olsun.
  ...
  **Durum (23 Eylül 2026) — ilk adım bitti, kutu açık:** `core::trim_curve` / `core::extend_curve` (`src/core/src/trim_curve.cpp`) çizgi, yay ve dairede; yay budanınca yay, daire iki kesimden budanınca yay kalıyor, çizgi eğrinin gerçek kesişiminde duruyor; UZAT yayın ucunu çemberi boyunca taşıyor. Sınır: `sinir=` (çok), seçim ya da hızlı budama (`hepsi`). Art arda tıklamalar tek çalışma, tek geri alma; tıklanan parça önizlemede komutun hesabıyla çiziliyor. Kanıt: `test_command.cpp` (`BUDA iki sınır arasındaki orta parçayı…`, `…yayı doğruyla budar…`, `…daireyi iki kesimden…`, `…ÜZERİNDE bitirir…`, `UZAT yayın ucunu çemberi boyunca…`, `…hızlı modda…`, `…TEK geri alma adımıdır`), `test_proof.cpp` (BUDA ×2, UZAT eşitlik kanıtı), `test_preview.cpp`, `test_curve_path.cpp`. **Kalan:** kesişim adaylarının (teğet dahil) önizlemede işaretlenmesi; ÇİT ile toplu budama ve toplu önizleme; tutulacak bölümü tıklama (ters taraf); geçici sınır uzantısı.
