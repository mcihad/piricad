...
5.20 NEVER hand-write or hand-edit the agent tool surface, a tool input schema, `docs/llms.txt` or `docs/llms-full.txt`. The surface a client is served is projected from `Registry` at run time (`ai::build_catalog`) and is never checked in; the two documents are generated by `kentos_docgen` and carry a do-not-edit first line. A second description of a command is a second answer to one question (§2.3, and this is 5.10 applied to the AI surface).
...
6.14 A change to ANY field of a `CommandSpec` or a `ToolSpec` — `.id`, `.names`, `.category`, `.params` (including a parameter's help, its arity, its word list or its range), `.undo`, `.flags`, `.summary` — is finished only when `make reference` has been run, all four generated artefacts are in the same commit (`docs/komutlar/referans.md`, `docs/nesneler/referans.md`, `docs/llms.txt`, `docs/llms-full.txt`) and `scripts/ci-gate-docs.sh` is green. The smallest change to a parameter changes what a client is told it may send, so the surface and its description are regenerated together or not at all. The gate regenerates and diffs; it fails rather than skipping when the generator is not built, because a freshness check that skips has never run.
