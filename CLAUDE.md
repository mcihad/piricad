...
|---|---|---|---|
| 8.1 | Partly lifted. The QRhi backend EXISTS behind `PIRICAD_WITH_RHI`, which is OFF by default: shaders are baked with `qsb` at build time, `MapCanvas` takes `QRhiWidget` as its base under that option, and the GPU path draws polygon fills, GPU-widened strokes and the overlay with **zero caller changes** — which is the claim this row was written to test. The default build is still the `QPainter` backend | What the GPU path does not draw yet is TEXT: `render.md` R8 wants an msdfgen SDF atlas shaped with HarfBuzz + FreeType, and R8's absence is what keeps the option off rather than on. `handles()` refuses every type it cannot draw rather than claiming it. `<rhi/qrhi.h>` also lives in Qt Gui's private headers, which most distributions package separately | Land the SDF text atlas, default `PIRICAD_WITH_RHI` ON once found, delete the QPainter backend by end of Phase 1 (`render.md` P3) |
