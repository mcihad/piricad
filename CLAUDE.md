...
|---|---|---|---|
| 8.1 | Partly lifted. The QRhi backend EXISTS behind `PIRICAD_WITH_RHI`, which is OFF by default: shaders are baked with `qsb` at build time, `MapCanvas` takes `QRhiWidget` as its base under that option, and the GPU path draws polygon fills, GPU-widened strokes and the overlay with **zero caller changes** — which is the claim this row was written to test. The default build is still the `QPainter` backend Text has landed too, behind `PIRICAD_WITH_TEXT`: the R8 atlas is real — msdfgen fields over FreeType outlines, shaped with HarfBuzz — and rulers, scale bar, north arrow and the drawing's own captions come out of it. What the GPU path does NOT draw yet is PATTERN FILLS, published raster symbols and marker lines, which is most of what MPYY prints; `handles()` refuses every type it cannot draw rather than claiming it | Land the pattern and raster-symbol passes, default both options ON once found, delete the QPainter backend by end of Phase 1 (`render.md` P3) |
