...
|---|---|---|---|
| 8.1 | Canvas is a `QPainter`-backed `MapCanvas`, not `QRhiWidget` | `qsb` (qt6-shadertools) is not installed, so `.qsb` shader packs cannot be baked | Install qt6-shadertools, bake shaders, land the QRhi backend behind the existing `render::Backend` with **zero caller changes**; delete the QPainter backend by end of Phase 1 (`render.md` P3) |
| 8.2 | No external dependency beyond Qt 6 — GDAL, PROJ, GEOS, CGAL, Lua, Python absent | Not installed on the development machine | Each stays behind `PIRICAD_WITH_<NAME>`, default OFF, hard-failing with an actionable message when ON but missing; the option flips to a supported configuration as its port is pinned in `/vcpkg.json`. PROJ-gated tests report **pending**, never passing (`data.md` Enforcement) |
...
8.4 Because of 8.1, `piricad_render` links no Qt today: the scene builder, view transform and backend interface are Qt-free, and the `QPainter` implementation of `render::Backend` lives in `/src/app`. The QRhi backend restores the `render -> Qt Gui` edge of Article 3.2.

8.5 Every rule in every rulebook is written against the target design and binds the stand-in too. A rule is waived for a stand-in only by a row in the table above naming that rule id.
