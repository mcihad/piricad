...
5.8 NEVER let a POSITION originate in model text. Every point in a generated command is a handle a recorded tool-call result minted, or a RELATIVE POINT — such a handle moved by a stated dimension, millimetres east and north — and the audit record names both (`konum_kaynagi`). A dimension is not a coordinate: a distance, a radius, a side length or an offset from a handle is what §5.2.5 calls `ofset_hesapla`, and OFSET's distance was always one. **This amendment supersedes the former wording** "every `Mm`/`Point2` in a generated command traces to a recorded tool-call result", which read literally forbade every dimension and, with no read tool minting a point handle, left no agent able to draw a single line (§5.2.5).
...
| 8.2 | Partly lifted. **Linked and on**: Qt 6, nlohmann/json, spdlog + fmt, doctest, Google Benchmark, GDAL, PROJ, libpqxx (Article 2.9), Clipper2, CDT, **libdxfrw** (`KENTOS_WITH_DXFRW`, ON wherever the pinned source can be fetched; the DXF road of io.md R13), **SARibbon** (MIT, pinned by commit in `cmake/KentOSCadDependencies.cmake`, the shell's ribbon of ui.md R46 — linked into `kentos_cad` alone and REQUIRED with it, like Qt Widgets: a shell without its ribbon lays out no commands, so Article 9's "behind `KENTOS_WITH_<NAME>`, default OFF" does not apply to it; this is named here rather than implied). **Linked behind an option**: CPython 3.14 + pybind11 (`KENTOS_WITH_PYTHON`, Article 8.3), LibreDWG (`KENTOS_WITH_DWG`), **CGAL** (`KENTOS_WITH_CGAL`, ON wherever CGAL 5.6+ is found; the planar arrangement of `core/planar.hpp` behind SINIR — this supersedes the former listing of CGAL as installed but not linked). **Pinned but not linked**: xxHash. **Installed but not linked**: GEOS, libxml2. **Removed**: Lua 5.4 + sol2, deleted with the amendment of §4.1 — one embedded language | Article 2.7 now requires the mature library wherever one exists, so this row shrinks with every integration rather than expiring at once | Each remaining library stays behind `KENTOS_WITH_<NAME>`, hard-failing with an actionable message when ON but missing, and defaulting ON once found. The row is deleted when GDAL, GEOS and CGAL are linked. A gated test reports **pending**, never passing (`data.md` Enforcement) |
